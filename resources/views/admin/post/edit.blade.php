                ...
                <div class="card">
                    ...
                    <div class="card-body">
                        ...
                        <div class="form-group @if($errors->has('name')) has-error @endif">
                            {!! Form::label('Tiêu đề') !!}
                            {!! Form::text('title', $post->title, ['class' => 'form-control', 'placeholder' => 'Tiêu đề']) !!}
                            @if ($errors->has('title'))
                                <span class="help-block text-danger">{!! $errors->first('title') !!}</span>@endif
                        </div>
                        <div class="form-group">
                            <p>Hình ảnh</p>
                            <img id="showimage" width="200px"height="auto" src="{{asset('/storage/post/'.$post->image)}}" alt="">
                        </div>
                        ...
                        <div class="form-group @if($errors->has('details')) has-error @endif">
                            {!! Form::label('Nội dung') !!}
                            {!! Form::textarea('details',$post->details, ['class' => 'form-control', 'placeholder' => 'Nội dung']) !!}
                            @if ($errors->has('details'))
                                <span class="help-block">{!! $errors->first('details') !!}</span>@endif
                        </div>
                        <div class="form-group">
                            {!! Form::label('Đặc biệt') !!}
                            {!! Form::select('especially', [1 => 'Có', 0 => 'Không'], isset($post->especially) ? $post->especially : null, ['class' => 'form-control']) !!}
                        </div>
                        <div class="form-group">
                            {!! Form::label('Publish') !!}
                            {!! Form::select('is_published', [1 => 'Hiển thị', 0 => 'Chờ'], isset($post->is_published) ? $post->is_published : null, ['class' => 'form-control']) !!}
                        </div>
